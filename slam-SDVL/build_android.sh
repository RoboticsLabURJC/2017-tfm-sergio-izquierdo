...
echo "Configuring and cross compiling SLAM_SDVL for Android ..."
...
do
  ...
  #cmake -DUSE_ANDROID=ON -DCMAKE_TOOLCHAIN_FILE=../android/android.toolchain.cmake -DANDROID_NDK=$HOME/Android/Sdk/ndk-bundle -DCMAKE_BUILD_TYPE=Release -DANDROID_ABI=$arch -DANDROID_NATIVE_API_LEVEL=android-26 ..
	cmake -DUSE_ANDROID=ON -DCMAKE_TOOLCHAIN_FILE=../android/android.toolchain.cmake -DANDROID_NDK=$HOME/android-ndk-r15b -DCMAKE_BUILD_TYPE=Release -DANDROID_ABI=$arch -DANDROID_NATIVE_API_LEVEL=android-26 ..
  cmake --build . -- -j4  
  cd ..
done
